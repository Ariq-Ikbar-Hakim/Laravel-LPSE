    ...
    <div class="py-8 px-4 md:px-8 font-jakarta bg-slate-100 dark:bg-slate-950 min-h-screen text-slate-850 dark:text-slate-100 transition-colors duration-300">
        <div class="max-w-7xl mx-auto space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Transfer Wewenang & Jabatan</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Ubah hak akses atau jabatan (admin, PPK, PP) secara sepihak untuk pengguna aktif.</p>
                </div>
            </div>
            ...
            <div class="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm">
                <form action="{{ route('admin.users.transfer-jabatan') }}" method="GET" class="flex flex-col sm:flex-row gap-3">
                    ...
                    @if(request('search'))
                        <a href="{{ route('admin.users.transfer-jabatan') }}" class="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl text-sm font-semibold transition text-center">
                            Clear
                        </a>
                    @endif
                </form>
                        ...
                        <table class="w-full text-left border-collapse text-sm">
                            <thead>
                                <tr class="border-b border-slate-100 dark:border-slate-800 text-slate-400 dark:text-slate-500 font-semibold text-xs uppercase bg-slate-50/50 dark:bg-slate-800/50">
                                    ...
                                    <th class="p-4">OPD</th>
                                    <th class="p-4 pr-6 text-center">Ubah Jabatan Wewenang</th>
                                </tr>
                            </thead>
                                ...
                                @foreach($users as $user)
                                    <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition">
                                        ...
                                        <td class="p-4 pr-6 text-center">
                                            <!-- Update Role Form -->
                                            <form action="{{ route('admin.users.update-role', $user) }}" method="POST" class="flex items-center justify-center space-x-2">
                                                @csrf
                                                @method('PATCH')
                                                ...
                                                <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-1.5 px-3 rounded-xl text-xs transition duration-150 shadow-sm">
                                                    Update
                                                </button>
                                            </form>
                                        </td>
